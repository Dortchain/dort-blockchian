...
from Dort.ssl.create_ssl import generate_ca_signed_cert, get_Dort_ca_crt_key, make_ca_cert
from Dort.util.bech32m import encode_puzzle_hash
from Dort.util.config import (
    create_default_Dort_config,
    initial_config_file,
    load_config,
    save_config,
    unflatten_properties,
)
from Dort.util.ints import uint32
from Dort.util.keychain import Keychain
from Dort.util.path import mkdir
from Dort.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'Dort keys generate'")
        return None
...
def create_all_ssl(root: Path):
    ...
    Dort_ca_crt, Dort_ca_key = get_Dort_ca_crt_key()
    Dort_ca_crt_path = ca_dir / "Dort_ca.crt"
    Dort_ca_key_path = ca_dir / "Dort_ca.key"
    Dort_ca_crt_path.write_bytes(Dort_ca_crt)
    Dort_ca_key_path.write_bytes(Dort_ca_key)
    ...
    Dort_ca_crt, Dort_ca_key = get_Dort_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, Dort_ca_crt, Dort_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            if Dort_init(root_path) == 0 and root_path.exists():
                return init(create_certs, root_path)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return Dort_init(root_path)


def Dort_version_number() -> Tuple[str, str, str, str]:
...
def Dort_minor_release_number():
    res = int(Dort_version_number()[2])
    print(f"Install release number: {res}")
    return res


def Dort_full_version_str() -> str:
    major, minor, patch, dev = Dort_version_number()
    return f"{major}.{minor}.{patch}{dev}"


def Dort_init(root_path: Path):
    if os.environ.get("Dort_ROOT", None) is not None:
        print(
            f"warning, your Dort_ROOT is set to {os.environ['Dort_ROOT']}. "
            f"Please unset the environment variable and run Dort init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Dort directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if Dort_ROOT is set, or if user has run Dort init twice
        # before a new update.
        check_keys(root_path)
        print(f"{root_path} already exists, no migration action taken")
        return -1

    create_default_Dort_config(root_path)
    create_all_ssl(root_path)
    check_keys(root_path)
    print("")
    print("To see your keys, run 'Dort keys show --show-mnemonic-seed'")
