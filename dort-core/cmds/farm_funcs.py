...
from Dort.cmds.units import units
from Dort.consensus.block_record import BlockRecord
from Dort.rpc.farmer_rpc_client import FarmerRpcClient
from Dort.rpc.full_node_rpc_client import FullNodeRpcClient
from Dort.rpc.wallet_rpc_client import WalletRpcClient
...
from Dort.util.misc import format_bytes
from Dort.util.misc import format_minutes
from Dort.util.network import is_localhost
...
async def summary(rpc_port: int, wallet_rpc_port: int, harvester_rpc_port: int, farmer_rpc_port: int) -> None:
    ...
    if amounts is not None:
        print(f"Total Dort farmed: {amounts['farmed_amount'] / units['Dort']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['Dort']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['Dort']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'Dort start wallet' and 'Dort wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'Dort wallet show'")
    else:
        print("Note: log into your key using 'Dort wallet show' to see rewards for each key")
