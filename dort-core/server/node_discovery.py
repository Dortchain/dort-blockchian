...
from Dort.protocols import full_node_protocol, introducer_protocol
from Dort.protocols.protocol_message_types import ProtocolMessageTypes
from Dort.server.address_manager import AddressManager, ExtendedPeerInfo
from Dort.server.address_manager_store import AddressManagerStore
from Dort.server.outbound_message import NodeType, make_msg
from Dort.server.server import DortServer
from Dort.types.peer_info import PeerInfo, TimestampedPeerInfo
from Dort.util.hash import std_hash
from Dort.util.ints import uint64
...
class FullNodeDiscovery:
    ...
    async def on_connect(self, peer: ws.WSDortConnection):
        if (
    ...
    async def update_peer_timestamp_on_message(self, peer: ws.WSDortConnection):
        if (
            peer.is_outbound
            and peer.peer_server_port is not None
            and peer.connection_type is NodeType.FULL_NODE
    ...
    async def _introducer_client(self):
        ...
        async def on_connect(peer: ws.WSDortConnection):
