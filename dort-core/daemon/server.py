...
from Dort.cmds.init_funcs import Dort_init
from Dort.daemon.windows_signal import kill
from Dort.server.server import ssl_context_for_root, ssl_context_for_server
from Dort.ssl.create_ssl import get_mozilla_ca_crt
from Dort.util.Dort_logging import initialize_logging
from Dort.util.config import load_config
from Dort.util.json_util import dict_to_json_str
from Dort.util.path import mkdir
from Dort.util.service_groups import validate_service
from Dort.util.setproctitle import setproctitle
from Dort.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Dort.")
    quit()
...
service_plotter = "Dort plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "Dort": "Dort",
        "Dort_wallet": "start_wallet",
        "Dort_full_node": "start_full_node",
        "Dort_harvester": "start_harvester",
        "Dort_farmer": "start_farmer",
        "Dort_introducer": "start_introducer",
        "Dort_timelord": "start_timelord",
        "Dort_timelord_launcher": "timelord_launcher",
        "Dort_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("Dort daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered Dort_ROOT
    os.environ["Dort_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with Dort_ROOT: {os.environ['Dort_ROOT']}")
