...
ROOT = pathlib.Path(importlib.import_module("Dort").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `Dort` file name (which is the executable)
    and the `Dort` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `Dort` directory, since the names collide.
...
version_data = copy_metadata(get_distribution("Dort-blockchain"))[0]
...
# TODO: collapse all these entry points into one `Dort_exec` entrypoint that accepts the server as a parameter

entry_points = ["Dort.cmds.Dort"] + [f"Dort.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "Dort.util.bip39"])

if THIS_IS_WINDOWS:
    Dort_mod = importlib.import_module("Dort")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/Dort/util/english.txt", "Dort/util"))
datas.append((f"{ROOT}/Dort/util/initial-config.yaml", "Dort/util"))
datas.append((f"{ROOT}/Dort/wallet/puzzles/*.hex", "Dort/wallet/puzzles"))
datas.append((f"{ROOT}/Dort/ssl/*", "Dort/ssl"))
...
add_binary("Dort", f"{ROOT}/Dort/cmds/Dort.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/Dort/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/Dort/server/start_{server}.py", COLLECT_ARGS)
