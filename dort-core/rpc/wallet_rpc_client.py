...
from Dort.pools.pool_wallet_info import PoolWalletInfo
from Dort.rpc.rpc_client import RpcClient
...
class WalletRpcClient(RpcClient):
    """
    Client to Dort RPC, connects to a local wallet. Uses HTTP/JSON, and converts back from
    JSON into native python objects before returning. All api calls use POST requests.
    Note that this is not the same as the peer protocol, or wallet protocol (which run Dort's
    ...
    async def log_in(self, fingerprint: int) -> Dict:
        try:
            return await self.fetch(
                "log_in",
                {"host": "https://backup.Dort.net", "fingerprint": fingerprint, "type": "start"},
            )
    ...
    async def log_in_and_skip(self, fingerprint: int) -> Dict:
        try:
            return await self.fetch(
                "log_in",
                {"host": "https://backup.Dort.net", "fingerprint": fingerprint, "type": "skip"},
            )
        except ValueError as e:
            return e.args[0]
